...
const LOCAL_NETWORK_ORIGIN_PATTERN = /^http:\/\/(?:(?:10|127)\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[0-1])\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}):5173$/;
...
const ACTIVE_ROOM_TTL_MS = Number(process.env.ROOM_TTL_MS || 60 * 60 * 1000);
const FINISHED_ROOM_TTL_MS = Number(process.env.FINISHED_ROOM_TTL_MS || 10 * 60 * 1000);
...
app.use((request, response, next) => {
  const normalizedOrigin = normalizeOrigin(request.headers.origin);
  if (isAllowedOrigin(normalizedOrigin)) {
...
app.get("/", (_request, response) => {
  response.json({
    ok: true,
    service: "cpl-socket-server",
    description: "CPL realtime Socket.IO backend",
    endpoints: {
      health: "/health",
      stats: "/stats",
      socket: "/socket.io/",
...
app.get("/health", (_request, response) => {
  response.json({
    ...
    rooms: rooms.size,
    teamRooms: teamRooms.size,
    cleanup: roomCleanupStats(),
  });
});

app.get("/stats", (_request, response) => {
  response.json(serverStats());
});
...
function isAllowedOrigin(origin) {
  return !origin || ALLOWED_ORIGINS.includes(origin) || LOCAL_NETWORK_ORIGIN_PATTERN.test(origin);
}

function roomCleanupStats() {
  return {
    activeRoomTtlMinutes: Math.round(ACTIVE_ROOM_TTL_MS / 60000),
    finishedRoomTtlMinutes: Math.round(FINISHED_ROOM_TTL_MS / 60000),
    intervalMinutes: Math.round(ROOM_CLEANUP_INTERVAL_MS / 60000),
  };
}

function shouldExpireRoom(room, now) {
  const ttl = room.phase === "finished" ? FINISHED_ROOM_TTL_MS : ACTIVE_ROOM_TTL_MS;
  return now - (room.updatedAt || room.createdAt || now) > ttl;
}

function cleanupStaleRooms() {
  const now = Date.now();
  let removedSingleRooms = 0;
  let removedTeamRooms = 0;

  for (const [code, room] of rooms.entries()) {
    if (!shouldExpireRoom(room, now)) continue;
    for (const player of room.players) {
      socketToRoom.delete(player.id);
    }
    rooms.delete(code);
    io.to(code).emit("room:error", { message: "Room expired because it was inactive." });
    io.in(code).socketsLeave(code);
    removedSingleRooms += 1;
  }

  for (const [code, room] of teamRooms.entries()) {
    if (!shouldExpireRoom(room, now)) continue;
    for (const player of room.players) {
      socketToTeamRoom.delete(player.id);
    }
    teamRooms.delete(code);
    io.to(`team:${code}`).emit("team:error", { message: "Team room expired because it was inactive." });
    io.in(`team:${code}`).socketsLeave(`team:${code}`);
    removedTeamRooms += 1;
  }

  if (removedSingleRooms || removedTeamRooms) {
    console.log(`CPL room cleanup removed ${removedSingleRooms} single rooms and ${removedTeamRooms} team rooms.`);
  }
}

function serverStats() {
  const publicTeamRooms = Array.from(teamRooms.values()).filter((room) => room.visibility === "public").length;
  const privateTeamRooms = teamRooms.size - publicTeamRooms;
  const singlePlayers = Array.from(rooms.values()).reduce((total, room) => total + room.players.length, 0);
  const teamPlayers = Array.from(teamRooms.values()).reduce((total, room) => total + room.players.length, 0);
...
setInterval(cleanupStaleRooms, ROOM_CLEANUP_INTERVAL_MS).unref();
